...
- checkpointer: 每遇到检查点时刻，就执行共享缓存刷盘，包括：WAL日志，事务提交日志、事务元信息等所有缓存。只刷盘，不会驱逐缓存。从而确保检查点之前的WAL日志不再需要，可以清理。
    - 触发机制：
      ...
      - `max_wal_size`超大触发
      - 执行复制触发：`pg_basebackup, pg_start_backup()`
      - 执行恢复时触发
...
WAL buffer刷盘时机：
1. 事务同步提交（synchronous_commit = 'on'）。lsn，每一个page都有一个pd_lsn，用于对比确认内存中的事务是否提交
...
为了避免这种事情的发生，PG引入了冻结freezing操作。PG9.4以后，冻结操作会设置表行的`t_infomask`的`XMIN_FROZEN`位，如果表行的xmin < current_txn_id - `vacuum_freeze_min_age`
