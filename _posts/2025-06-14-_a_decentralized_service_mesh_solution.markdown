...
Envoy，Traefik这样的代理软件不具备服务注册/发现的能力。这一部分功能通常依赖于第三方软件系统。例如，早期Spring Cloud使用Erueka做服务注册中心，后来用Consul。甚至Istio，Linkerd干脆基于Kubernetes实现。

不管是Eureka，Consul，还是Kubernetes，这些系统都是需要单独部署的软件系统，增加了部署复杂度和成本。

而且，如果依赖了Consul或者etcd（Kubernetes）这样的强一致系统，会可能因为网络分区问题而影响到功能的可用性。

## Serf

Serf是Hashicorp公司的一个去中心化的服务发现软件。Serf本身依赖于memberlist库，采用gossip协议与集群其他成员通信，是一个AP属性的最终一致性系统。

Serf一个比较巧妙的设计是，对于集群成员变更等事件的本地处理，是基于Shell脚本的。在这个Shell环境中，我们可以定义脚本，还可以实现更复杂的可执行程序，來处理事件。好处就是中立性：
- 语言中立：具体的服务可以用任何编程语言实现。对比Spring Cloud这样的侵入式SDK，Serf更加灵活。
- 工具中立：集群成员发生变化后，我们可以修改DNS服务器记录，也可以更新本地`/etc/hosts`文件。当然在这里，我们会更改反向代理软件的配置。

# 组合方案

## 服务注册

Serf的集群是去中心化的。各个节点允许发布自己的标签。利用这个方式，我们可以发布各自节点上的服务信息。例如，使用标签`service=app1,app2`，让集群其他成员知晓该节点上有服务app1和app2可用。

```
serf tags -set service="app1,app2" -delete service # 删除已经存在的svc标签，并增加新的svc标签。
```
...
```shell
$> serf members
vm1  192.168.122.128:7946  alive  zone=a,service=app1
vm2  192.168.122.116:7946  alive  zone=b,service=app2
...
```
...
相比Kubernetes，这套系统更为简单（或者简陋）。有一些功能缺失，需要动手能力比较强的团队来弥补：
- **健康检查**。这个方案里没有涉及健康检查。因为本地运行服务可以是多样的，可以是基于Systemd，也可以使用Docker/Podman等容器方案自带的Healthcheck设施。统一的要求是，能够周期性检查本地所有服务，并及时更新serf agent的tag信息。
